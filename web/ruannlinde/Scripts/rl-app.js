(function (angular) {
	...
	var runApp = function ($location) {
		...
		console.log('$$html5:', $location.$$html5);
		//var appUrl = 
	};
	...
	var logService = function ($http, $q, $location) {
		'use strict';
		...
		return {
			GetLog: function () {
				return $http({
					url: $location.$$absUrl + 'Home/GetLogContent',
					method: 'GET'
				});
			},
			ClearLog: function () {
	...
	angular.module('rlApp', ['ngRoute', 'ngStorage', 'ngFileUpload', 'ngAnimate'])
		...
		.factory('logService', ['$http', '$q', '$location', logService])
